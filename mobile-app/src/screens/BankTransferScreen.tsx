...
export function BankTransferScreen({ navigation }: Props) {
  ...
  const submit = () => {
    ...
    if (!accountNumber.trim()) {
      Alert.alert('Account number needed', 'Enter the receiving account number to continue.');
      return;
    }
    const result = bankTransfer(numericAmount, bank, accountName);
    if (!result.success) {
      Alert.alert('Transfer not completed', result.message);
      return;
    }
    Alert.alert('Bank transfer complete', `${formatPHP(numericAmount)} sent to ${accountName}.`, [{ text: 'Done', onPress: () => navigation.goBack() }]);
  };
  ...
  return <Screen backgroundColor={colors.blue} contentStyle={styles.content}>
    ...
    <Pressable onPress={() => { setBank('BDO'); setAccountName('Maria Santos'); setAccountNumber('0012345678'); }} style={({ pressed }) => [styles.favorite, pressed && styles.pressed]}><View style={styles.plus}><Text style={styles.plusText}>+</Text></View><Text style={styles.addText}>{bank ? 'Favorite loaded' : 'Add demo favorite'}</Text></Pressable>
    <Text style={styles.sectionLabel}>Account to Transfer</Text>
    <SelectField label="Account type" value="PHP" onPress={() => Alert.alert('Account type', 'PHP selected for this demo.')} />
