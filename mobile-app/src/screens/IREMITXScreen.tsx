  ...
  return (
    <Screen backgroundColor={colors.blue} contentStyle={styles.screenContent} scroll>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.formCard}><Text style={styles.formTitle}>Recipient details</Text><SelectField label="Send to" value={country} onPress={() => setCountry(countries[(countries.indexOf(country) + 1) % countries.length] ?? countries[0] ?? '')} /><FormField label="Recipient full name" value={recipient} onChangeText={setRecipient} placeholder="Enter full name" autoCapitalize="words" /><FormField label="Amount to send" value={amount} onChangeText={setAmount} keyboardType="numeric" placeholder="0.00" helper="Service fees and exchange rates are shown before confirmation." /><View style={styles.rateRow}><Text style={styles.rateLabel}>Estimated delivery</Text><Text style={styles.rateValue}>Within minutes</Text></View><PrimaryButton title="Continue remittance" icon="arrow-forward" onPress={submit} /></View>
