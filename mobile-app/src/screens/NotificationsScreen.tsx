...
export function NotificationsScreen({ navigation }: Props) {
  return (
    <Screen backgroundColor={colors.blue} contentStyle={styles.screenContent}>
      ...
      <View style={styles.content}><View style={styles.headerRow}><Text style={styles.heading}>Recent updates</Text><Text style={styles.unread}>1 unread</Text></View><View style={styles.list}>{notifications.map((item) => <Pressable accessibilityRole="button" key={item.title} onPress={() => showAlert(item.title, item.body)} style={({ pressed }) => [styles.row, pressed && styles.pressed]}><View style={[styles.icon, { backgroundColor: item.color }]}><Ionicons name={item.icon} size={19} color={colors.white} /></View><View style={styles.copy}><Text style={styles.title}>{item.title}</Text><Text style={styles.body}>{item.body}</Text><Text style={styles.time}>{item.time}</Text></View><Ionicons name="chevron-forward" size={16} color={colors.muted} /></Pressable>)}</View><PrimaryButton title="Mark all as read" icon="checkmark-done" onPress={() => showAlert('Notifications cleared', 'All notifications are marked as read.')} /></View>
    </Screen>
  );
}
